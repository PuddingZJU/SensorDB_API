...
def testPyInterf():
	...
	assert l2.compare(l2.sensordbSelectDataByActivityName(db, "摔倒后爬起".encode(encoding)),l2.LoadData(b"activity.txt"))
	...
	assert l2.compare(l2.sensordbSelectDataByOperatorName(db, "李嘉俊".encode(encoding)),l2.LoadData(b"operator.txt"))
	assert l2.compare(l2.sensordbSelectDataByPositionId(db, 3),l2.LoadData(b"position.txt"))
	assert l2.compare(l2.sensordbSelectDataByPositionName(db, "裤子侧口袋（左）".encode(encoding)),l2.LoadData(b"position.txt"))
	assert l2.compare(l2.sensordbSelectDataByActivityTypeId(db, 4),l2.LoadData(b"activitytype.txt"))
	assert l2.compare(l2.sensordbSelectDataByActivityTypeName(db, "摔倒".encode(encoding)),l2.LoadData(b"activitytype.txt"))
...
def testWrapperClass():
	...
	assert l2.compare(db.selectDataByActivityName("摔倒后爬起".encode(encoding)),l2.LoadData(b"activity.txt"))
	...
	assert l2.compare(db.selectDataByOperatorName("李嘉俊".encode(encoding)),l2.LoadData(b"operator.txt"))
	assert l2.compare(db.selectDataByPositionId(3),l2.LoadData(b"position.txt"))
	assert l2.compare(db.selectDataByPositionName("裤子侧口袋（左）".encode(encoding)),l2.LoadData(b"position.txt"))
	assert l2.compare(db.selectDataByActivityTypeId(4),l2.LoadData(b"activitytype.txt"))
	assert l2.compare(db.selectDataByActivityTypeName("摔倒".encode(encoding)),l2.LoadData(b"activitytype.txt"))
